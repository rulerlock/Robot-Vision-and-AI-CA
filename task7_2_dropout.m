...
varFrec = 50;
global dropout_rate
dropout_rate = 0.3; % Set dropout rate for training
dropout_rate_for_testing = 0;
...
for i = 1:epochs
    ...
    for j = 1:mini_batch_size:numImages
        ...
        % Forward propagation
        [AL, caches, dropout_masks] = model_forward(X_batch, params, dropout_rate);
        
        % Backward propagation to calculate gradients
        grads = model_backward(AL, Y_batch, caches, dropout_masks, dropout_rate);
    ...
    if mod(i, varFrec) == 0
        % Training metrics
        [AL, ~, ~] = model_forward(trainFeatures, params, dropout_rate);
        ...
        % Test metrics
        [AL, ~, ~] = model_forward(testFeatures, params, dropout_rate_for_testing);
...
function [AL, caches, dropout_masks] = model_forward(X, params, dropout_rate)
    caches = {};
    dropout_masks = {};
    A = X;
    L = length(params);
    ...
    for i = 1:L - 1
        ...
        % Activation
        [A, activation_cache] = activation_function(Z, 'leaky_relu');
        
        % Dropout
        if dropout_rate > 0
            % Create dropout mask
            D = rand(size(A)) > dropout_rate;
            A = A .* D; % Apply mask to activations
            A = A / (1 - dropout_rate); % Scale activations to keep expected value constant
            dropout_masks{end + 1} = D;
        else
            dropout_masks{end + 1} = ones(size(A)); % No dropout during testing
        end
...
function [dA_prev, dW, db] = linear_activation_backward(dA, cache, activation_type, D, dropout_rate)
...
function grads = model_backward(AL, Y, caches, dropout_masks, dropout_rate)
    grads = {};
    L = length(caches);
    dAL = AL - Y; % Derivative of softmax with cross-entropy
    [dA, dW, db] = linear_activation_backward(dAL, caches{L}, 'swish', dropout_masks{L - 1}, 0); % Output layer without dropout
    grads{end + 1} = {dA, dW, db};

    for l = L - 1:-1:1
        [dA, dW, db] = linear_activation_backward(dA, caches{l}, 'swish', dropout_masks{l}, dropout_rate);
        grads{end + 1} = {dA, dW, db};
    end
end
